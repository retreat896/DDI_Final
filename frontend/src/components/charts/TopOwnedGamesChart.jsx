...
function TopOwnedGamesChart({ userGames }) {
  ...
  useEffect(() => {
    axios.get(`${API_BASE}/api/analytics/top-owned?limit=15`)
      .then(res => {
        setDbGames(res.data);
      })
  ...
  function draw(data) {
    ...
    const x = d3.scaleLinear()
      .domain([0, d3.max(data, d => +(d.owners_midpoint || 0)) * 1.1])
      .range([0, width]);
    ...
    svg.selectAll('rect')
      ...
      .attr('width', d => x(+(d.owners_midpoint || 0)));
  ...
  if (loading) return <p style={{ color: '#64748b' }}>Loading top games from database…</p>;
