 ...
 * seed-d1.mjs
 * -----------
 * Seeds the LOCAL Cloudflare D1 database (.wrangler/state/...).
 * Skips any table that already has data (idempotent).
 *
 * Uses the same approach as seed-d1-remote.mjs:
 *   1. Generate a full SQL file from the CSV (single-row INSERTs)
 *   2. Execute ONE `wrangler d1 execute --local --file=<sql>` call per table
 *
 * This avoids spawning wrangler thousands of times and is much faster.
 *
 * Usage:  npm run seed
 *
 * Est. time: ~60–90 seconds total
 */
...
/** Run a wrangler d1 command against the LOCAL database. */
...
/** Check if a table already has rows in the local DB. Returns count. */
function localRowCount(table) {
  try {
    const out    = wranglerLocal(`execute ${DB_NAME} --local --command "SELECT COUNT(*) AS c FROM \\"${table}\\";"`);
...
async function generateSql(csvFile, tableName) {
  ...
  const skipped = allCols.length - importCols.length;
  if (skipped) console.log(`    (skipping ${skipped} large-HTML columns: ${[...SKIP_COLS].join(', ')})`);
...
for (const stmt of indices) {
  try {
    ...
    wranglerLocal(`execute ${DB_NAME} --local --command "${escaped}"`);
