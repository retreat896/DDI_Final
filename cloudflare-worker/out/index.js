...
var jsonErr = /* @__PURE__ */ __name((msg, status = 500) => Response.json({ error: msg }, { status }), "jsonErr");
...
__name(openIdParams, "openIdParams");
...
__name(resolveSteamId, "resolveSteamId");
...
app.get("/api/auth/callback", async (c) => {
  ...
  const match2 = claimedId.match(/https:\/\/steamcommunity\.com\/openid\/id\/(\d+)/);
  if (!match2) return Response.redirect(`${frontendUrl}?error=authentication_failed`, 302);
  const steamId = match2[1];
  const profile = env.STEAM_API_KEY ? await fetchProfile(steamId, env.STEAM_API_KEY) : { steamid: steamId };
  ...
  try {
    ...
    const enriched = results.map((r) => {
      ...
      return {
        ...
        review_pct: total > 0 ? Math.round(pos / total * 1e3) / 10 : null,
        total_reviews: total
      };
